...
**2026-05-02 — GitHub→Netlify auto-deploy + display polish**
GitHub OAuth flag cleared and Netlify GitHub App installed (Only select repositories: `jeffcalvert.net`). Build settings: branch `main`, build command `hugo`, publish directory `public`, env var `HUGO_VERSION=0.159.0`. Auto-deploy verified — push triggers build in ~60s. Workflow guide written at `Migration/SITE_WORKFLOW.md`.

Display work:
- **Pandemic Diary template**: dedicated single template (`layouts/pandemic-diary/single.html`) with the standard "(Pandemic Diary – day N)" subtitle linking to the section index, hr separator, no date/reading-time line. Section assigned `type: pandemic-diary` via `cascade` in `_index.md` so the template lookup works.
- **Pandemic Diary list**: dedicated list template (`layouts/pandemic-diary/list.html`) sorts oldest-first.
- **Prev/next nav**: re-pointed for PD so left arrow → older, right arrow → newer (Hugo's `.PrevInSection` is the chronologically older page; `.NextInSection` is newer — we use them directly with that meaning). Site-wide nav now flex-justified left/right with a 2rem gap so long titles can't collide.
- **Blockquote cleanup**: stripped `> ` ` ``` ` ` ` ` ``` ` lines from 7 migrated posts (33 fenced quote blocks total). Migration script had wrapped quoted text in code fences, causing scrollable code-block rendering instead of normal blockquotes. Pattern was paired open/close fences inside blockquotes — clean delete via sed.
- **Image lightbox**: new render hook (`layouts/_default/_markup/render-image.html`) wraps every markdown image in `<a class="lightbox" href="...">`. Lightbox partial (`layouts/partials/lightbox.html`) provides the overlay, close (×), prev/next (‹ ›), keyboard nav (Esc/←/→), and cycles through `a.lightbox` anchors within the article. Lightbox uses `.open` class (not `[hidden]` attribute) for visibility — `[hidden]` had specificity issues that left the lightbox visible on initial load. Hover scale-up on images removed.
- **Image captions**: render hook also produces `<figure>`/`<figcaption>` when alt text is present. Convention: `![caption](image.jpg)`. Bulk-rewrote 73 lines from old `![](file.jpg) caption` pattern using a perl one-liner. Captions render centered, in Arial.
- **Link hover**: replaced theme's animated `scaleX(0)` underline with instant `text-decoration: underline` (`a:not(nav a):before { display: none }` kills the animated pseudo-element).
- **format taxonomy enabled**: added `format = "format"` to `[taxonomies]` in `hugo.toml`, generating `/format/essay/`, `/format/adventure-report/`, `/format/rough/` index pages automatically. `rushofitall/` section uses a custom list template (`layouts/rushofitall/list.html`) that surfaces "Browse by format: …" links at the top, dynamically sourced from `.Site.Taxonomies.format` so new formats appear automatically.
- **Internal links use RelPermalink**: site-wide overrides for `_default/list.html`, `_default/taxonomy.html`, `_default/terms.html`, plus `partials/breadcrumb.html` and project's `single.html` swap `.Permalink` → `.RelPermalink`. Reason: with `baseURL = https://jeffcalvert.net/` set in `hugo.toml`, Hugo's dev server doesn't reliably rewrite absolute URLs in localhost preview, so internal links were jumping to the live site. Canonical links and og:url remain absolute (SEO).
